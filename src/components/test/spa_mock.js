// !!! mock !!!!
if (!top.pathConfig) {
  window.sessionStorage.setItem("childPagination", true);
  top.pathConfig = {}
  // top.pathConfig.application = "vxpact";
  // top.pathConfig.application = "person";
  // top.pathConfig.application = "vxfinance";
  // top.pathConfig.application = "vxhr";
  // top.pathConfig.application = "vxproject";
  // top.pathConfig.application = "market";
  // top.pathConfig.application = "features";
  // top.pathConfig.application = "health";
  // top.pathConfig.application = "documents";
  // top.pathConfig.application = "market";
  // top.pathConfig.application = "auth";
  // top.pathConfig.application = "vxpatent";
  // top.pathConfig.application = "vxasset";
  // top.pathConfig.application = "config";
  // top.pathConfig.application = "demo";
  top.pathConfig.application = "retail";
  // top.pathConfig.application = "vxhr";
  // top.pathConfig.application = "features";
  // top.pathConfig.application = "zhxq";
  // top.pathConfig.application = "daq";
  // top.pathConfig.application = "zjoa";
  // top.pathConfig.application = "yjz";
  // top.pathConfig.application = "vip";
  // top.pathConfig.application = "oa";
  // top.pathConfig.application = "file";
  // top.pathConfig.application = "log";
  // top.pathConfig.application = "corp";
  // top.pathConfig.application = "sso";
  // top.pathConfig.application = "fyzhmd";
  // top.pathConfig.application = "config";
  // top.pathConfig.application = "factory";

  top.user = {
    create_time: "2019-08-11 22:30:15",
    create_user: "admin",
    del_flag: "否",
    dept_no: "01011103",
    email: "1@qq.com",
    id_card: null,
    login_control: "PC,IOS,ANDROID",
    mobile: null,
    modify_time: "2019-08-12 10:30:15",
    modify_user: "wangyh",
    parent_dept_no: null,
    real_name: "王永宏",
    user_disp: "wangyh/王永宏",
    // user_no: "wangyh",
    // user_no: "yaojz",
    user_no: "admin",
    user_state: "正常",
    user_type: "外部员工",
    _dept_info: {
      biz_path: null,
      create_time: "2019-07-03 11:46:48",
      create_user: "wangyh",
      create_user_disp: "/wangyh",
      del_flag: "否",
      dept_class: "dept_type_02",
      dept_full_name: null,
      dept_head: "nantaifuzeren",
      dept_level: null,
      dept_name: "斐济公司",
      dept_no: "101701",
      dept_tel: null,
      dept_type: "真实",
      id: 7,
      is_leaf: "是",
      modify_time: "2019-08-01 16:25:23",
      modify_user: "wangyh",
      modify_user_disp: "/wangyh",
      parent_no: "010101",
      path: "/0001/0101/010101/01010101/",
      path_name: null,
      remark: null,
      reserve_col1: "continet_005",
      reserve_col2: "153"
      // reserve_col1: null,
      // reserve_col2: null 
    }
  }

  sessionStorage.setItem("bx_auth_ticket", "xabxdzkj-a2f8e27f-eb1f-4562-94a8-8e3e2cdbf5b4");
}


export default function dummy () {

}