<template>
  <div class="tree-grid">
    ...
    <el-row type="flex" class="row-bg" :justify="gridButtonJustify">
...
export default {
  ...
  methods: {
    ...
    //   } else if ("edit" == type) {
    //     if (
    //       operate_item.operate_service &&
    //       Object.prototype.toString.call(operate_item.operate_service) !==
    //         "[object String]"
    //     ) {
    ...
    //         operate_item.operate_service || operate_item.service_name;
    //     }
    ...
    deleteRowData(rowData, exeservice) {
      ...
      this.deleteData(dataArray, exeservice).then(() => {
    ...
    mergeAdd(parentKeyVal, keyVal, addedRow) {
      ...
      function innerAddFunc(addedNode) {
        ...
        let parents = this.gridData.filter(
          (item) => item[this.noCol] == parentKeyVal
        );
        if (parents.length > 0) {
    ...
    mergeUpdate(oldRow, newRow) {
      if (
        newRow.hasOwnProperty(this.parentCol) &&
        newRow[this.parentCol] != oldRow[this.parentCol]
      ) {
        // parent changed
